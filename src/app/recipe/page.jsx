...
export default function Recipes() {
  ...
  const foodCat = foods.filter(food =>
   food.title.toLowerCase().includes(searchFood.toLowerCase()) || food.category.toLowerCase().includes(searchFood.toLowerCase() ))
...
<Link href={`/chinese/${food.id}`} title={`${food.category} of ${food.title}`}>
  <p>
    <span className='ml-[250px] mt-[550px] text-black text-[12px] font-bold bg-[gold] rounded p-[5.8px] tracking-[2px]'>{food.category}</span>
   </p>
   ...
   <h2 className='font-semibold tracking-[2.8px]'>{food.title}</h2>
    ...
    <p className='flex flex-row gap-2 ml-[110px] mt-[11px] items-center'><FaRegClock />
